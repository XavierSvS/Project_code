...
for j = 1:length(d)
    if Ts ~= 0
        % discrete-time
        if Dss == 0
            D(:,:,j) = Css*((S(:,:,j)*S2)*sum(ST./(ones(n,n,length(d))-d(j)*deig),3))*Css'; %(ST.*(d(j)*deig))
        else
            D(:,:,j) = Css*((S(:,:,j)*S2)*...
                       (sum(ST./(ones(n,n,length(d))-d(j)*deig),3)+...
                        sum(ST./((ones(n,n,length(d))-d(j)*ones(n,n,length(d))).*(ones(n,n,length(d))-deig)),3)))*Css';
        end
    else
        if Dss == 0
        % continuous-time
            D(:,:,j) = Css*((S(:,:,j)*S2)*sum(-ST./(d(j)*ones(n,n,length(d))+deig),3))*Css';
        else
        end
    end
end
